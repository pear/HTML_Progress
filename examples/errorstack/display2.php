...
class Log_display extends Log
{
    ...
    var $_printf = '<b>%s </b>: %s in <b>%s</b> (<b>%s</b> at line <b>%s</b>)';
    var $_ereg = 'in (.*) \(file (.*) at line (.*)\)';
                         ...
                         $level = PEAR_LOG_DEBUG)
    {
        ...
        if (!empty($conf['printf'])) {
            $this->_printf = $conf['printf'];
        }        	

        if (!empty($conf['ereg'])) {
            $this->_ereg = $conf['ereg'];
        }        	
    }
    ...
    function log($message, $priority = null)
    {
        ...
        if (ereg($this->_ereg, $message, $matches)) {

            $message = str_replace($matches[0], '', $message);
            echo $this->_error_prepend .
                @sprintf($this->_printf, ucfirst($this->priorityToString($priority)), 
                        $message, $matches[1], $matches[2], $matches[3], $matches[4]) .
